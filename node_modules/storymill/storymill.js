...
exports.writeFile = function (params) {
    if (params.type === 'pdf') {
        console.log('\n' + params.name + '.pdf создаётся...\n');

        writePdf(params.name);
    } else {
        fs.writeFileSync(smCfg.project.export + params.name + '.' + params.type, replaceResources(params.content), encoding = 'utf8');
...
var copyStaticFiles = function (techs) {
    techs.forEach(function (tech) {
        var cmd = 'cp ' + smCfg.project.root + 'desktop.bundles/book/_book.' + tech + ' ' + smCfg.project.export;
...
var writePdf = function (fileName) {
    ...
    var pdfPrinterParams = smCfg.pdfPrinter[pdfPrinter].params;
...
var apacheFop = function (fileName, params) {
    var confFile = smCfg.pdfPrinter.apachefop.conf;
    var pdfFile = [smCfg.project.export, fileName, '.pdf'].join('');
    var xmlFile = [smCfg.project.root, 'node_modules/storymill/1.xml'].join('');
    var xslFile = [smCfg.project.root, 'node_modules/storymill/apachefop.fo.xsl'].join('');

    var args = ['-c', confFile, '-xml', xmlFile, '-xsl', xslFile, '-pdf', pdfFile].join(' ');
    var cmnd = ['fop', args].join(' ');

    console.log(cmnd);
    
    sys.exec(cmnd, function(err) {
        console.log(err ? err : '\n\n' + fileName + '.pdf создан.\n');
    });
}
