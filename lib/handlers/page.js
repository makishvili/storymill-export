...
module.exports = inherit(require('../handler'), {
    _onDataJSON : function(dataJSON) {
        ...
        return Vow.all([bemJSONHtmlPromise, bemJSONFb2Promise, this._getTemplate()]).spread(function(bemJSONHtml, bemJSONFb2, template) {
            var fileName = this._req.query.id || false;
...
*/
    ...
    _getBemJSON : function(data, type) {
        ...
        if (type === 'html') {
            ...
            bemJson = assetsCfg
                        .getPageAsset(this._path, 'priv-fb2')
                        .then(function(priv) {
                            priv['i-global'].params(this._getGlobalParams());
                            var res = priv['b-page'](data);
                            return !!this._req.param('body')? res.content : res;
